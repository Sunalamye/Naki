...
## 12. 合法性分層與 AI 輸入語意（2026-08-01）

> §11 修的是「動作送不出去」。本節修的是**更上游的兩件事**：
> 誰有權決定動作合法、以及模型收到的輸入是不是它訓練時的語意。

### 12.1 起因：一次漏和

實測某手 oplist = `[discard, riichi, tsumo]`，Mortal 回傳 `dahai`，
送出端忠實把和牌牌打掉了。

當時的紀錄只寫「Mortal 自己選的」——**那個判讀是錯的**。
根因不是模型選錯，而是分層錯誤：**「能不能和」的權威在伺服器，模型只該評估「和的價值」**。
當時沒有任何一層在把關。
...
Mortal / Core ML（只負責對候選動作評分）
        ↓
AutoPlayDecisionResolver（終局保護 · 模式閘門 · 去重）
        ↓
...
- oplist 含 `tsumo`/`ron` 時，`.auto` **一律送和牌，凌駕 AI 推薦**
- `.recommend` 只顯示不送出；`.off` 完全不送
- 推薦的動作必須真的在這批 oplist 裡，否則是拿舊推薦操作新 oplist
- 送出前再次比對 `sequence/seat/source/contextTile`，過期決策一律作廢
- **缺 oplist 一律 fail closed**

一併移除舊的「打牌不等 oplist、直接送出」路徑。那是 §11 為了讓打牌會動而加的，
現在看它同時是漏和的幫兇：伺服器正提供自摸時，它會繞過一切直接盲送打牌。

**刻意做成 `nonisolated` 純函式**（不碰 UI／網路），才能對
「同一批 oplist + 同一組推薦」寫回歸測試。13 個測試涵蓋漏和重播、
三種模式的送出次數、快照過期、動作不在 oplist、座位不符、副露無推薦時的 pass。

### 12.3 MortalSwift 的四個狀態機錯誤

| 問題 | 實際影響 |
|------|---------|
| 向聽剪枝用「當前值」而非下界 | 當前值隨遞迴只會遞減（是上界），且 target=4 時根節點初值 9 ≥ minShanten 初值 8 → **整個遞迴一次都沒跑，任何手牌都回傳 7**。等於向聽計算完全沒有運作 |
| `shanten == -1 && !atFuriten` | 振聽規則上**只限制榮和**，這裡卻讓振聽擋掉自摸 |
| 大明槓／暗槓沒更新 `tehaiLenDiv3` | 吃／碰有扣、槓漏了；補嶺上後手牌淨少一組，不扣則之後所有向聽用錯的組數算 |
| 吃／碰／槓後沒重算向聽與等待 | 原本只在開局／摸牌／打牌時算，副露後仍沿用舊值，使 mask 與 observation 過期 |

第一項單純調大初值**不能解決**——深層節點仍會誤剪，必須換成真正的下界。

### 12.4 更上游：模型輸入語意未經驗證

`mortal.mlmodelc` 是固定成品，1012 個 channel 各代表什麼由 libriichi 決定。
v0.3.0 把 libriichi 換成純 Swift 時**沒有做過任何逐 channel 的等價驗證**。

實測（同一串 MJAI 事件同時餵兩邊，在**每一個需要動作的時點**比對）：
...
| **現況** | **0（已移植區段 888 格：ch0–876、878–888）** | **0** |

其餘 124 格（ch877、ch889–1011）需要單人期望值表與役種判定，尚未移植；
這些格子在 Swift 版**保留正確格數但填 0**——格數對是關鍵，
內容缺失只是資訊少，格數錯則是其後全部語意錯位。

佈局對齊後才浮出來的狀態機錯誤（每一個都會單獨造成錯誤決策）：
向聽剪枝漏算搭子使聽牌被算成一向聽、四枚同牌的假單騎、
`shanten` 在摸牌後被重算而失去比較基準、立直成立沒扣 1000 點、
吃碰槓掛到被吃那家的河上。

取捨、完整落差清單與方法見 `MortalSwift/docs/decisions/obs-parity.md`。

> **測資本身也要驗。** 第一版對拍劇本只有三個事件，大半 channel 的「一致」
> 其實只是「兩邊都是 0」。換成走過三巡、含碰與他家立直的完整劇本後，
> 立刻多冒出 6 個真實落差。沒被執行到的路徑，測試不會告訴你它是錯的。

> ⚠️ **未移植的那 124 格代表 Bot 仍拿不到「每張打牌的期望值」這組資訊。**
> 已移植區段對拍全綠代表輸入語意不再錯位，但資訊仍不完整，
> 推薦品質應視為「可用但未達 libriichi 水準」。

### 12.5 方法論（本輪用戶反覆指正）

三次都是同一件事：**該去拿現成的權威資料，而不是自己算**。

| 我做的 | 應該做的 |
|--------|---------|
| 憑舊碼猜 Liqi 欄位編號 | 查 `docs/protocol/liqi.json` |
| 掃描像素、自建座標校準來標牌 | 讀 shader uniform（`_MainTex_ST` 直接告訴你在畫哪張牌） |
| 用「非零 channel 數」估算編碼落差 | 拿 libriichi 當 oracle 逐格對拍 |

第三項的推論還得出了嚴重高估的錯誤結論（「只填 23 格」，實際只差 82 格）。

**判準：動手算之前先問「產生這個資料的那一方有沒有留下來」。**
遊戲要畫出牌就一定持有牌的位置；要送請求就一定有協定定義；
模型要吃 observation 就一定有產生它的編碼器。

---

## 13. 介面不得呈現不生效的控制項（2026-08-01）
...
| 位置 | 症狀 | 處置 |
|------|------|------|
| Bot 狀態列 | 三麻對局顯示 `Mortal (3P)`，但實際推論用的是四麻模型 | 改為 `Mortal (4P) ⚠️ 三麻無專用模型` |
| 進階設定 · 位置校準 | 三個滑桿，`updateCalibration()` 是空的 `// TODO`，且目標是 Laya 時代的座標點擊 | 移除 |
| 進階設定 · 推薦溫度 | Slider 只綁 `@State` 沒有 `onChange`；**而且 MortalSwift 根本沒有 boltzmann 取樣** | 移除 |
| 進階設定 · 旋轉高亮 | 程式碼註解自己寫著「Unity 下不會產生任何視覺變化」 | 移除 |

推薦溫度那個特別值得記：它不是「接線斷了、接回去就好」，
而是**在控制一個不存在的功能**。查證時如果只看 UI 層會誤判成「補個 onChange 就行」。

### 判準

**讓使用者以為自己調整了什麼、其實沒有，比沒有那個選項更糟。**

前者會讓人把後續的行為差異歸因到那個設定上，形成錯誤的心智模型，
而且很難自己發現——因為沒有任何錯誤訊息。

### 界線：明確失敗 ≠ 靜默無效

MCP 的 6 個高亮工具在 Unity 下同樣無法運作，但**保留不動**。差別在於：

- MCP 工具回傳的是**明確的失敗**（`unity-client-no-laya`），呼叫端拿得到清楚的錯誤
- UI 開關是**靜默無效**，使用者拿不到任何回饋

所以判準不是「不能運作就刪掉」，而是「**不能運作又不說**才要刪掉」。
API 可以誠實地說「我做不到」；一個開關做不到這件事。

### 連帶：對外宣稱也適用同一條

同一輪核對 README 時，發現公開說法與實際不符的地方：

- Naki README 寫「macOS 14.0+」，實際部署目標是 26.0——照著做裝不起來
- MCPWebKit / WebViewBridge README 寫「macOS 26.0+」，但已降到 macOS 14 / iOS 17
- MCPKit / MCPWebKit / WebViewBridge 的 `version` 常數與發布的 tag 全部對不上
- MortalSwift 英文 README 停在 v0.2.0，還在說用 Rust FFI（兩版前就移除）

**取捨**：`version` 常數已改成與**現有 tag** 一致，但**不重發 tag**。
嚴格說已發布的 tag 指向的 commit 裡常數仍是舊值；
但那三個常數沒有實際讀取者，重發 tag 造成的擾動大於收益。
