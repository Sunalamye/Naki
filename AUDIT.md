...
- mask（46 格）：**完全一致**
- obs（1012 channel）：**82 個不一致**，性質是**佈局錯位**而非缺段

已對齊前 28 格（82 → 71）。取捨、方法與剩餘工作見
`MortalSwift/docs/decisions/obs-parity.md`。

> ⚠️ **在 obs 對拍全綠之前，Bot 的推薦品質不可當真。**
> 這比 §11 修的送出面問題更根本：送出面壞掉是「動作出不去」，
> 輸入語意錯是「模型在對它沒見過的分布外插」。
