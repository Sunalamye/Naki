...
//  async 的（`start_game` 中間要 `await createNativeBot`）。最直覺的寫法
//  `Task { @MainActor in await handle(event) }` 每則事件開一個獨立 Task，
//  而**獨立 Task 之間沒有任何順序保證**：
//
//      start_game → Task A（await createNativeBot，暫停）
//      tsumo      → Task B（可能先跑完）
//
//  結果是事件亂序、甚至在 bot 建立完成前就被消費。主路徑（`NakiWebCoordinator`）
//  已經改成「單一 buffered AsyncStream + 單一 consumer」；Legacy 路徑
//  （`LegacyWebViewCoordinator`）先前還留著每則一個 Task 的舊寫法。
//
//  這個型別把那個模式抽成兩邊共用的一份：
...
//  而 handler 本身要在 MainActor 上跑（它會碰 ViewModel）。
...
nonisolated final class SerialEventIntake {
    ...
    /// 關閉入口：consumer 迴圈會在消化完已排隊的事件後結束
    func finish() {
        continuation.finish()
    }
}
