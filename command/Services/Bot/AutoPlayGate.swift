...
/// 自動打牌的觸發閘門
///
/// 純函式，所有閘門在同一個 switch 裡，可以逐條單測。把判斷散回非同步的重試框架與
/// 送出通道裡，會變成**看不出新加的條件會擋掉哪條既有路徑**——例如「沒有 oplist 就別
/// 觸發」跟「有對局就等待」互相打架造成死結、「此刻推薦是空的」被當成「模型決定不做」
/// 而把該碰的牌蓋成過。這兩個都不是單一條件寫錯，是耦合造成的。
///
/// 這裡**只決定要不要動、動什麼**；怎麼送、重試幾次、去抖多久在 `AutoPlayEngine`。
/// 那些跟送出通道與 log 通道綁死，搬過來只會把耦合換個位置。
enum AutoPlayGate {
