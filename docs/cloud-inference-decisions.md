...
## D21. （2026-08-10）立直第二段 429 不開斷路器：rate limit ≠ 伺服器掛

**來源**：Akagi #227 第二項（上游自己標為「需驗證」、尚未修）。立直兩段式是
毫秒內連發兩個 request；伺服器若對 burst 做 rate limit，第二段會吃 429。
原本的 `resolveReachDiscard` catch 對**任何**錯誤都 `breaker.recordFailure`，
於是一次 429 → 斷路器開 5s→120s 退避 → 接下來幾手全部跳過雲端。
四麻是靜默降級成本地；三麻雲端-only 是**接下來幾手全部無推薦**。

**決定**：429 特判——短暫等待重試一次（`Retry-After` 解得出且 ≤1s 就照它，
否則 0.4s；決策逾時預算才 2s，等更久不如退回本地）；二連 429 這一手退回
本地（D3 不變），但**不開斷路器**——rate limit 的伺服器是可達的。
非 429 的失敗維持原語意：開斷路器。

**驗證**：`CloudBotTests` 三條（429 重試成功沿用雲端捨牌／二連 429 退本地且
下一手照打 API／500 開斷路器且退避內不再打），mutation 各自驗過
（429 併入通用 catch → 兩條 429 測試紅；拔掉 500 的 recordFailure → 500 測試紅）。
**無 live 對局驗證**——上游也還沒證實伺服器真的會對 burst 回 429。

## D22. （2026-08-10）強制手本地回答，不花 API call

**來源**：Akagi #227 第一項／#228。完整合法動作集是單元素時（最常見＝立直後
每一摸的強制摸切），答案完全確定，伺服器只可能回同一手——原本每一摸仍燒
一次 API call 加一次 2s 逾時風險。

**決定**：`BotReaction` 加 `forced`，由 `BundledCoreMLBot` 在**截斷前的
mask** 上量（`mask.filter==1` 恰一格；推薦數量分不出「強制」與「mapper 只
映射出一個」——Akagi 同款判準）。`CloudBot` 4p 路徑見 `forced` 直接回本地
反應：不打 API、不動斷路器、不動 fallbackStreak（這不是 fallback）。

**邊界**：鳴牌視窗（`[pon, pass]` 等）合法集恆 ≥2，照常問雲端。副露後的
合成推薦路（`recommendationsFromCurrentMask`）不量 forced——那條的 mask 有
多層合成 fallback，不是引擎決策當下的合法集快照。三麻雲端-only 沒有本地
mask 可量，不在本輪範圍（立直後伺服器是否還送 oplist 未驗證）。

**驗證**：`CloudBotTests` 強制手零 API 呼叫＋下一個非強制決策照常問雲端；
