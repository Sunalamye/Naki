...
Majsoul 有**三種獨立**的閒置機制，不要混淆：

| 機制 | 觸發場景 | 閾值 | 結果 | 解決方法 |
|------|---------|------|------|---------|
| **大廳閒置登出** | 在大廳/牌局外長時間無動作 | 50 分鐘 | 強制登出 | `clientHeatBeat()` |
| **遊戲中單次操作超時** | 在牌局中輪到你時不操作 | 依房間設定 | 自動出牌/跳過 | 無法阻止（伺服器控制）|
| **遊戲中連續超時 (AFK)** | 連續多次操作超時 | 連續 N 次 | 進入托管 + 可能封禁 | 無法阻止（伺服器控制）|
...
當玩家在遊戲中**連續多次**操作超時，伺服器會：
1. 將玩家狀態改為非 `READY`（顯示斷線/托管圖標）
2. 發送 `NotifyPlayerConnectionState` 通知其他玩家
3. 啟用自動摸切模式（不只是單次超時的自動出牌）
4. 如果持續 AFK，發送 `NotifyAFKResult` 可能觸發臨時封禁
...
enum GamePlayerState {
    ...
    AUTH = 1;     // 認證中
    SYNCING = 2;  // 同步中
    READY = 3;    // 正常（只有這個狀態不顯示托管圖標）
}

// AFK 結果通知（連續超時後的處罰）
message NotifyAFKResult {
    uint32 type = 1;         // AFK 類型
    uint32 ban_end_time = 2; // 封禁結束時間（Unix 時間戳）
    string game_uuid = 3;    // 遊戲 UUID
}
```
...
```javascript
// 玩家連線狀態陣列（4 個玩家）
view.DesktopMgr.player_link_state  // [0, 0, 0, 0]

// 狀態枚舉
view.ELink_State.NULL     // 0
view.ELink_State.AUTH     // 1
view.ELink_State.SYNCING  // 2
view.ELink_State.READY    // 3 (正常狀態)
```

#### 與機制 2 的區別

| 項目 | 機制 2（單次超時） | 機制 3（連續超時 AFK） |
|------|-------------------|----------------------|
| 觸發條件 | 單次操作超時 | 連續多次操作超時 |
| 處理方式 | `OperationTimeOut()` 自動出牌一次 | 進入持續托管模式 |
| 玩家狀態 | 保持 READY | 變為非 READY |
| UI 顯示 | 無特殊顯示 | 顯示斷線/托管圖標 |
| 後果 | 只影響當次操作 | 可能觸發臨時封禁 |

**重要**：這個機制完全由伺服器控制，客戶端無法阻止。唯一的解決方法是在遊戲中保持活躍操作。
